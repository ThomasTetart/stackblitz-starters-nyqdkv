...
import {map, merge, Observable, Subject, tap, withLatestFrom} from "rxjs";
...
export class Step3Component {
  ...
  createQuiz(): void {
    if (this.formQuiz.valid && this.idCategory) {
      ...
      this.questions$ = merge(this.quizService.switchQuestion,this.quizService.createQuiz(this.idCategory.toString(), this.formQuiz.get('selectDifficulty')?.value as Difficulty));
    }
  }
